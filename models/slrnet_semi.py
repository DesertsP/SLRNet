...
class Net(SingleStageNet):
    ...
    def forward(self, x, x_raw=None, cls_labels=None, seg_labels=None, indicators=None, single_scale=False):
        ...
        cls_2 = self.classifier_forward(x2_aug, mask2_aug)
        ...
        reg_loss *= cls_labels[:, :, None, None].type_as(reg_loss)
        ...
        # pseudo_indicators = ~indicators
        seg_loss1 = balanced_mask_loss_ce(x_aug, pseudo_gt, cls_labels)
        seg_loss2 = balanced_mask_loss_ce(x2_aug, pseudo_gt, cls_labels)
        seg_loss_aux = balanced_mask_loss_ce(x_aux, pseudo_gt, cls_labels)
        ...
        # classification loss
        cls_loss1 = F.multilabel_soft_margin_loss(cls, cls_labels, reduction='none')
        cls_loss2 = F.multilabel_soft_margin_loss(cls_2, cls_labels, reduction='none')
        cls_loss_aux = F.multilabel_soft_margin_loss(cls_aux, cls_labels, reduction='none')
