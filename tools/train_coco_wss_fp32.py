...
from datasets.sampler import DistributedWeightedRandomSampler, DistributedSamplerWrapper, WeightedRandomSampler
...
class Trainer(object):
    def __init__(self, model, train_set, val_set, config, local_rank=-1, visualize=True):
        ...
        # dataset, balance sample
        assert hasattr(train_set, 'sample_weights')
        if self.is_distributed:
            self.sampler = DistributedSamplerWrapper(DistributedWeightedRandomSampler(weights=train_set.sample_weights,
                                                                                      num_samples=config.train.num_samples,
                                                                                      replacement=True))
        else:
            self.sampler = WeightedRandomSampler(weights=train_set.sample_weights, num_samples=config.train.num_samples,
                                                 replacement=True)
    ...
    def train_epoch(self, epoch):
        ...
        for step, data in enumerate(self.train_loader):
            ...
            label = label.cuda()
            cls_loss, seg_loss, mask_reg_loss, route_reg_loss, masks, slots_update = self.model(img.cuda(),
                                                                                                img_raw.cuda(), label)
            cls_loss = cls_loss.mean()
            seg_loss = seg_loss.mean()
            mask_reg_loss = mask_reg_loss.mean()
            route_reg_loss = route_reg_loss.mean()
            loss = config.train.loss_coeff[0] * cls_loss + \
                   config.train.loss_coeff[1] * mask_reg_loss + \
                   config.train.loss_coeff[2] * route_reg_loss
            ...
            if self.local_rank <= 0:
                progbar.update(step + 1,
                               values=[("loss", reduced_loss.item()),
                                       ("cls", cls_loss.item()),
                                       ("seg", seg_loss.item()),
                                       ("reg1", mask_reg_loss.item()),
                                       ('reg2', route_reg_loss.item())])
    ...
    def visualize(self, step):
        ...
        # image_raw = F.interpolate(image_raw, size=masks['cam'].size()[-2:], mode="bilinear", align_corners=True)
        vis_list = [image_raw, mask_rgb(masks['cam'], image_raw, n=202),
                    mask_rgb(masks['dec'], image_raw, n=202)]
        pseudo_mask = mask_rgb(masks["pseudo"], image_raw, n=202)
        ...
        # print(label.shape)
        summary_grid = make_grid(vis_elements, label, class_names=self.train_loader.dataset.CLASSES)
